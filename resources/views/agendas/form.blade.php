<div class="bg-white p-8 rounded-lg shadow-md">
    ...
    <form action="{{ $mode === 'create' ? route('agendas.store') : route('agendas.update', $agenda->id) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="date" name="meeting_date" id="meeting_date" value="{{ old('meeting_date', $agenda->meeting_date?->format('Y-m-d')) }}"
        ...
        <div>
            ...
            <input type="time" name="meeting_time" id="meeting_time" value="{{ old('meeting_time', $agenda->meeting_time?->format('H:i')) }}"
                   class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" required>
        </div>
        
        <div class="flex items-center gap-4 pt-4">
            <button type="submit" class="inline-flex items-center space-x-2 bg-{{ $mode === 'create' ? 'green' : 'yellow' }}-600 hover:bg-{{ $mode === 'create' ? 'green' : 'yellow' }}-700 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition duration-300 ease-in-out">
