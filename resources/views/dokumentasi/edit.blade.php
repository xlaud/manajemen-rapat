...
<div class="bg-white p-8 rounded-lg shadow-md">
    ...
    <form action="{{ route('dokumentasi.update', $dokumentasi->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
        @method('PUT')
        <div>
            <label for="title" class="block text-sm font-medium text-gray-700 mb-1">Judul Dokumentasi</label>
            <input type="text" name="title" id="title" value="{{ old('title', $dokumentasi->title) }}"
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="5"
                      class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                      placeholder="Masukkan deskripsi dokumentasi">{{ old('description', $dokumentasi->description) }}</textarea>
        </div>
        <div>
            <label for="file" class="block text-sm font-medium text-gray-700 mb-1">Ganti File Dokumentasi (Opsional)</label>
            ...
            @if($dokumentasi->file_path)
                <p class="mt-2 text-sm text-gray-600">File saat ini: <a href="{{ route('dokumentasi.download', $dokumentasi->id) }}" class="text-blue-600 hover:underline">Download</a></p>
            @endif
        </div>
        <div>
        ...
        <div class="flex space-x-3">
            <button type="submit" class="flex items-center space-x-2 bg-yellow-600 hover:bg-yellow-700 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition duration-300 ease-in-out">
